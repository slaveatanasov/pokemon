...
export function query(args: { type?: string, after?: string; limit?: number; q?: string; }): Connection<Pokemon> {
  const { type, after, q, limit = 8 } = args;
  ...
  const filterByType: (as: Pokemon[]) => Pokemon[] =
    type === undefined
      ? identity
      : A.filter(pokemon => pokemon.types.toLocaleString().toLowerCase().split(',').includes(type.toLowerCase()));
